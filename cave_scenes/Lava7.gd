...
"the top fire":$"%FireBtn2",
"the glass": $"%Glass",
"the second glass":$"%Glass2",
"another weak wall":$"%BreakableWall1",
"the padlock":$"%PuzzleLock",
"the ring":$"%Ring",
"the ladder":$"%Ladder",
"the green crystal":$"%GreenCystal",
"the side wall":$"%side_wall",
...
#
	...
	GlobalVars.active_map = "Mid7MapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Yes! A padlock keeps things safe.")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The ring I was carrying has vanished.")		
	else:
		if can_save:
			print("shelves wrong")
