...
onready var game_objects = {
	...
	"the secret button":$"%SecretPush",
	"the second button": $"%second push btn",
	"the hidden button":$"%the hidden button",
	"the hammer":$"%Hammer",
	"the ladder":$"%Ladder",
	"a weak wall":$"%a weak wall",
	"a breakable wall":$"%a breakable wall",
	"the weak floor":$"%BreakableFloor",
	"the pen":$"%Pen",
	"the telescope":$"%Telescope",
	"the lever":$"%Lever",
	"the small lever":$"%the small lever",
	"the useful lever":$"%the useful lever"
...
#
	...
	GlobalVars.active_map = "Lower3MapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Yes! You can write stories and create your own worlds with a pen!")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer_a = Timer.new()
			add_child(yield_timer_a)
			yield_timer_a.start(5); yield(yield_timer_a, "timeout")			
			if was_removed:
				GlobalSignals.emit_signal("speak", "The telescope I was carrying has vanished. It must be some kind of magic item.")		
	else:
		if can_save:
			print("shelves wrong")
...
#			yield(get_tree().create_timer(4), "timeout")
			...
			yield_timer_b.start(4); yield(yield_timer_b, "timeout")			
...
#	setup_inventory_items()
	...
	if from_entry:
		var new_pos = Vector2(exit_door.position.x+400, exit_door.position.y+50)
...
#
#		yield(get_tree().create_timer(1.5), "timeout")
#		GlobalSignals.emit_signal("save_sentence", "had a fall")
#		_remove_speak()
...
func _on_DropLadderHere_body_entered(body):
	if body.name == "PlayerGirl":
		if game_objects.has("the ladder"):
			...
			GlobalSignals.emit_signal("speak", "I can use the ladder and put it here. Click on 'Use' and then click 2 times on the ladder.")
