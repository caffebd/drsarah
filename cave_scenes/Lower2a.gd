...
onready var game_objects = {
	"the first lever":$"%LeverBridge1",
	"the second lever":$"%LeverBridge2",
	"the pen":$"%Pen",
	"the bell":$"%Bell",
...
#
	...
	GlobalVars.active_map = "Lower2aMapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Yes, the sound of a bell can be a warning!")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The pen I was carrying has vanished. It must be some kind of magic item.")		
	else:
		if can_save:
			print("shelves wrong")
