...
"the alpha key":$"%KeyAlpha",
"push button":$"%PushBtn1",
"second push button":$"%PushBtn2",
"the beta door":$"%BetaDoor",
"the beta key":$"%BetaKey",
"the delta key":$"%DeltaKey",
"the log push button":$"%LogPushBtn",
"the lever":$"%Lever",
...
#
	...
	GlobalVars.active_map = "Water11MapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Yes! An umbrella gets wet so I can stay dry.")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The pot I was carrying has vanished.")		
	else:
		if can_save:
			print("shelves wrong")
