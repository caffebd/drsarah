...
#	"the log":$"%Log",
	"the push button":$"%PushBtn",
	"another push button":$"%PushBtn2",
	"yet another push button":$"%PushBtn3",
	"the mirror":$"%TheMirror",
	"the bell":$"%Bell",
	"the last push button":$"%the last push button",
	"the double push button":$"%DoublePush",
...
#
	...
	GlobalVars.active_map = "Lower2MapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Oh! When I look in the mirror, I see my twin!")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The bell I was carrying has vanished. It must be some kind of magic item.")		
	else:
		if can_save:
			print("shelves wrong")
...
#	setup_inventory_items()
	...
	if GlobalVars.carried_inventory.has("the ladder"):
		$"%LadderHint".queue_free()
		$"%DropLadderHere".queue_free()
		$"%GetTheLadder".queue_free()
...
func _button_pressed(btn):
	print ("in btn press "+btn)
	if btn == "Get":
		if game_objects.has("the ladder"):
			GlobalVars.the_player.tut_to_ladder_inventory()
			$"%LadderHint".queue_free()
			$"%DropLadderHere".queue_free()
			$"%GetTheLadder".queue_free()
	if btn=="ladderinv":
...
func _on_LadderHint_body_entered(body):
	if body.name == "PlayerGirl":
		if game_objects.has("the ladder"):
			
			GlobalVars.the_player.player_pause = true			
			GlobalSignals.emit_signal("speak", "To get the ladder click 'Get' and then click on the ladder.")
#			yield(get_tree().create_timer(5.5), "timeout")
			var yield_timer_c = Timer.new()
			add_child(yield_timer_c)
			yield_timer_c.start(5.5); yield(yield_timer_c, "timeout")
			GlobalSignals.emit_signal("speak","If you cannot see the ladder, you can click 'Get' followed by the ladder icon in the inventory…")
#			yield(get_tree().create_timer(6), "timeout")
			var yield_timer_a = Timer.new()
			add_child(yield_timer_a)
			yield_timer_a.start(6); yield(yield_timer_a, "timeout")
			GlobalSignals.emit_signal("speak","...it will come back to you. Maybe it’s magic.")
#			yield(get_tree().create_timer(3.5), "timeout")
			...
			if game_objects.has("the ladder"):
				...
				GlobalVars.tutorial_active=false
				GlobalVars.the_player.tut_done()
				GlobalVars.the_player.player_pause = false
				$"%LadderHintCol".disabled = true
...
func _on_GetTheLadder_body_entered(body):
	if body.name == "PlayerGirl" and game_objects.has("the ladder"):
		GlobalSignals.emit_signal("speak", "I should GET this ladder.")


func _on_DropLadderHere_body_entered(body):
	if body.name == "PlayerGirl":
		if game_objects.has("the ladder"):
			GlobalSignals.emit_signal("speak", "I should go back and get the ladder.")
		else:
			GlobalSignals.emit_signal("speak", "I can use the ladder and put it here. Click on 'Use' and then click on the ladder.")
			

func _on_DropLadderHere_body_exited(body):
	if body.name == "PlayerGirl":
		if !game_objects.has("the ladder"):
			GlobalSignals.emit_signal("speak", "I can use the ladder and put it here. Click on 'Use' and then click on the ladder.")
...
func _on_ButtonRemind_body_entered(body):
	if body.name == "PlayerGirl":
		GlobalSignals.emit_signal("speak", "I wonder if I should push the red button?")
