...
#
	...
	GlobalVars.active_map = "Lower1MapA"
...
#	delete items not in fb array
	...
	print ("from load player pos "+str(GlobalVars.temp_player_pos))
	setup_inventory_items()
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak","Wow! That opened the door. I wonder how that works?")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(6); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The mirror I was carrying has vanished. It must be some kind of magic item.")		
	else:
		if can_save:
			print("shelves wrong")
...
func _load_game(from_door: bool, from_entry:bool):
	...
	if room_items == null:
		...
		setup_inventory_items()
		GlobalSignals.emit_signal("fade_in")		
		game_objects["the log"].set_state("on")
...
#	setup_inventory_items()
	...
	print ("from door "+str(from_door))
	print ("entry " +str(from_entry))
	...
	print ("I LOADED AT "+str(GlobalVars.the_player.position))
...
func _button_pressed(btn):
	if btn == "Look at":
		GlobalVars.the_player.tut_to_sign($"%Sign".get_global_transform_with_canvas().origin)
	if btn == "tut_sign":
		GlobalVars.the_player.tut_done()
		GlobalVars.tutorial_active = false
#		GlobalSignals.emit_signal("speak", "Looks like I need to find an item that solves this riddle.")
	if btn == "the clockclicked":
		GlobalVars.the_player.tut_to_hole($"%Shelf".get_global_transform_with_canvas().origin)
	if btn == "Use":		
		GlobalVars.the_player.tut_to_clock()
	if btn == "Get" and removing_shelf_item:
		GlobalSignals.emit_signal("speak","Click 2 times on the mirror to take it back.")		
		GlobalVars.the_player.tut_to_hole($"%Shelf".get_global_transform_with_canvas().origin)
	if btn == "mirrorremove":
		GlobalSignals.emit_signal("speak", "I should try putting this clock in the hole.")
		GlobalVars.tutorial_active = true
		GlobalVars.the_player.tut_to_use()
		
func _on_hole_trigger_body_entered(body):
	if body.name == "PlayerGirl":
		if !sign_read:
			sign_read = true
			GlobalVars.the_player.player_pause = true
			GlobalSignals.emit_signal("speak","There’s something written on that sign. I should look at it.")
#			yield(get_tree().create_timer(4), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(4); yield(yield_timer, "timeout")
			GlobalVars.the_player.tut_to_look()



func _on_ClockTrig_body_entered(body):
	if body.name == "PlayerGirl":
		if game_objects.has("the clock"):
			GlobalSignals.emit_signal("speak","A clock has hands and keeps you organised. I should take this to that hole by the sign.")
...
func _on_CrystalTrigger_body_entered(body):
	if body.name == "PlayerGirl":
		if game_objects.has("the blue crystal"):
			GlobalSignals.emit_signal("speak", "Wow. This looks expensive. I should find someone to give it to.")
...
func _on_PutItem_body_entered(body):
	if body.name == "PlayerGirl":
		if game_objects.has("the clock"):
			GlobalSignals.emit_signal("speak", "Looks like I need to find an item that solves this riddle.")
		elif GlobalVars.shelf_placed_items[0] != "":
			removing_shelf_item = true
			GlobalVars.the_player.player_pause = true
			GlobalSignals.emit_signal("speak", "There is something in the hole already. I need to remove it first.")
#			yield(get_tree().create_timer(5.5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5.5); yield(yield_timer, "timeout")
			GlobalVars.tutorial_active = true
