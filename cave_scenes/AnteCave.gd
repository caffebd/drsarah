...
func _on_FallTrigger_body_entered(body):
	if body.name == "PlayerGirl" and first_time:
		first_time = false
...
#		yield(get_tree().create_timer(4), "timeout")
		...
		body._fade_out(false)
#		yield(get_tree().create_timer(4), "timeout")
		var yield_timer_2 = Timer.new()
		add_child(yield_timer_2)
		yield_timer_2.start(4); yield(yield_timer_2, "timeout")
		GlobalVars.load_from_door = true
		GlobalVars.load_from_entry = false
		get_tree().change_scene("res://cave_scenes/Lower1a.tscn")
