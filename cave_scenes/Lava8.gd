...
"the push button":$"%DoublePush",
"the ladder":$"%Ladder",
"the right lever":$"%RightLever",
"the top fire":$"%FireBtn",
"the small fire":$"%FireBtn2",
"the first glass":$"%Glass",
"the first lever":$"%RightLever",
"the second lever":$"%Lever",
"the third lever":$"%Lever2",
"the second glass":$"%Glass2",
"the lower button":$"%DoublePush2",
"the cooking pot":$"%CookingPot",
"the lamp":$"%Lamp",
...
onready var spawns = [$"%Spawn1", $"%Spawn2",$"%Spawn3",$"%Spawn4",$"%Spawn5"] 
# $"%Spawn3", $"%Spawn4"
var spawn_count = 0
...
#
	...
	GlobalVars.active_map = "Lava8MapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Yes! The cooking pot can take away my hunger.")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The lamp I was carrying has vanished.")		
	else:
		if can_save:
			print("shelves wrong")
