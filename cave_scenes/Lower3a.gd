...
onready var game_objects = {
	"the first lever":$"%Lever",
	"the ladder":$"%Ladder",
	"the second lever":$"%Lever2",
	"the third lever":$"%Lever3",
	"the first push button":$"%PushBtn1",
	"the sideways push":$"%SidePushBtn",
	"the last push button":$"%LastPush",
	"the compass":$"%Compass",
...
#
	...
	GlobalVars.active_map = "Lower3aMapA"
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Of course, a boomerang comes back to you when you throw it.")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The compass I was carrying has vanished.")		
	else:
		if can_save:
			print("shelves wrong")
