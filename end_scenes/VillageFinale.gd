...
onready var drone_sentence_panel = $"%Drone/DroneUIControl/SentencePanelDrone"

onready var speak_panel = $"%PlayerGirl/UIControl/SpeakBG"
onready var drone_speak_panel = $"%Drone/DroneUIControl/SpeakBG"

onready var speak_box = $"%PlayerGirl/UIControl/SpeakBG/SpeakFill/Margin/Speak"
onready var drone_speak_box = $"%Drone/DroneUIControl/SpeakBG/SpeakFill/Margin/Speak"
#onready var inventory_bar = $"%InventoryPanel"
onready var player = $"%PlayerGirl"
onready var drone = $"%Drone"

onready var droneUI = $"%Drone/DroneUIControl"
onready var playerUI = $"%PlayerGirl/UIControl"
...
onready var maps = {
	"VillageFinaleMap":$"%VillageFinaleMap"
}
...
"the beta key":$"%BetaKey",
"the alpha door":$"%AlphaDoor",
"the beta door":$"%BetaDoor",
"the large log":$"%Log"
...
func _ready():
	...
	GlobalSignals.connect("village_saved", self, "_village_saved")
	GlobalSignals.connect("remove_temp_wall", self, "_remove_temp_wall")
...
#
	...
	GlobalVars.active_map = "VillageFinaleMap"
...
func _remove_temp_wall():
	$"%TempWall".queue_free()
	$"%AllCrystalsColl".disabled = false
...
#			game_objects[item].extend = true
	...
	for game_item in game_objects:
		...
		if !room_item_keys.has (game_item):
			...
			if (game_item == "the large log"):
				print ("lever is broken")
				GlobalSignals.emit_signal("lever_broken", "the end lever", true)
				$"%BadGuyTrigger".queue_free()
...
#			_save_game()
		...
		if can_save:
			GlobalSignals.emit_signal("speak", "Yes, a ring has no beginning and no end. It just goes round and round forever.")
#			yield(get_tree().create_timer(5), "timeout")
			var yield_timer = Timer.new()
			add_child(yield_timer)
			yield_timer.start(5); yield(yield_timer, "timeout")
			if was_removed:
				GlobalSignals.emit_signal("speak", "The boomerang I was carrying has vanished.")		
	else:
		if can_save:
			print("shelves wrong")
...
func _on_DroneTrigger_body_entered(body):
	if body.name == "PlayerGirl" and drone_used == false:
		GlobalSignals.emit_signal("speak", "It’s a drone. Use the drone to fly places that Dr. Sarah cannot go.")
		var yield_timer_drone = Timer.new()
		add_child(yield_timer_drone)
		yield_timer_drone.start(5);
		yield(yield_timer_drone, "timeout")
		GlobalSignals.emit_signal("speak", "The drone can use levers and buttons. It can collect items and bring them to Dr. Sarah.")
		



func _on_BadGuyTrigger_body_entered(body):
	if body.name == "PlayerGirl":
		$"%BadGuyTrigger".queue_free()
		$"%ForceField".set_state("on")
		$"%EnemyLever".set_state("on")
		GlobalSignals.emit_signal("save_sentence", "trapped in force field")
		var yield_timer_log = Timer.new()
		add_child(yield_timer_log)
		yield_timer_log.start(5);
		yield(yield_timer_log, "timeout")
		$"%Log".y_pos_change = 1400
		$"%Log".start_y = -2000
		GlobalSignals.emit_signal("bad_guy_speak")
		$"%field_coll".disabled=false

func _on_FieldOff_area_entered(area):
	print ("BAD GUY HIT ME "+area.name)
	$"%ForceField".set_state("off")
	$"%BadGuy".i_will_get_you()
	GlobalSignals.emit_signal("lever_broken", "the end lever", true)
	var yield_timer_bg = Timer.new()
	add_child(yield_timer_bg)
	yield_timer_bg.start(5);
	yield(yield_timer_bg, "timeout")
	$"%Log".queue_free()
	$"%field_coll".disabled=true
	GlobalSignals.emit_signal("remove_from_game_objects", "the large log")
	GlobalSignals.emit_signal("save_sentence", "bad guy defeated")
	


func _on_HelpUs_body_entered(body):
	if body.name == "PlayerGirl" and not GlobalVars.carried_inventory.has("the alpha key"):
		$"%HelpUsAudio".play()
		GlobalSignals.emit_signal("speak", "Doctor Sarah, we’ve been locked in here. Please find the key to get us out but be careful!")
		GlobalSignals.emit_signal("save_sentence", "villagers trapped")


func _village_saved():
	GlobalSignals.emit_signal("speak", "Thank you. I don’t know what we would have done without you.")
	$"%saved1".play()
	$"%HelpUs".queue_free()
	remove_from_game_objects("help us")


	


func _on_saved1_finished():
	GlobalSignals.emit_signal("speak", "That man wanted to take over our village. We had to hide all our crystals in the caves.")
	$"%saved2".play()




func _on_saved2_finished():
	GlobalSignals.emit_signal("speak", "Maybe you can help us get them back? If you find any please put them in the correct holes.")
	$"%saved3".play()
	GlobalSignals.emit_signal("save_sentence", "villagers rescued")
	


func _on_AllCrystalsTrig_body_entered(body):
	if body.name == "PlayerGirl":
		$"%ThankYouCrystals".play()
		GlobalSignals.emit_signal("speak", "Thank you for finding all the crystals. I don’t know what we will do if that man returns.")
		GlobalSignals.emit_signal("save_sentence", "stay village")
