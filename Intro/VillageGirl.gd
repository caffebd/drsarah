...
#			if GlobalVars.current_sentence == "Give the medicine to the girl" and GlobalVars.girl_waiting:
#				GlobalSignals.emit_signal("button_pressed", object_text+"second")		
#	else:
	if GlobalVars.tutorial_active:
		if GlobalVars.current_sentence == "Give the medicine to" and GlobalVars.girl_waiting:
...
func _sentence_check(sentence):
	...
	match sentence:
		get:
			GlobalSignals.emit_signal("speak", "I can't take the girl!")
		give:
			return
...
#			GlobalSignals.emit_signal("sentence_clear")
		...
		use_on:
			return
		look:
			GlobalSignals.emit_signal("speak", "The girl looks really sick.")
		_:
			return
