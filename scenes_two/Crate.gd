...
#	connect("mouse_exited", self, "_on_exit")
	...
	GlobalSignals.connect("crate_box_broken", self, "_remove_me")
...
func _sentence_check(sentence):
	...
	match sentence:
		...
		use:
			GlobalSignals.emit_signal("speak", "Maybe I can break it open?")

		use_on:
			return
		"use the hammer on the crate":
			GlobalSignals.emit_signal("crate_box_broken")
		look:
			GlobalSignals.emit_signal("speak", "A wooden crate. I can see an apple inside.)
		_:
