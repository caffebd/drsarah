...
func set_state(new_state):
	...
	if state == "off":
		tween.tween_property(self, "rect_position", Vector2(move_to_x, move_to_y), 6.0 )
		tween.tween_callback(self, "_bad_guy")
	else:
		GlobalSignals.emit_signal("bad_guy_speak", "Ahhhhhh.....!!!!!!!")
		GlobalSignals.emit_signal("lever_broken", "the lever", true)
		GlobalSignals.emit_signal("remove_forcefield")
		
		tween.tween_property(self, "rect_position", Vector2(start_x, start_y), 3.0 )
		tween.tween_callback(self, "_check_can_remove")
		yield(get_tree().create_timer(1), "timeout")
		GlobalSignals.emit_signal("bad_guy_fire")
...
func _bad_guy():
	GlobalSignals.emit_signal("bad_guy_speak", """So you thought you could escape? Think again!
