...
export var object_text := "the log bridge"
...
func _sentence_check(sentence):
	...
	match sentence:
		...
		use_on:
			return
		"pull the old log":
			GlobalSignals.emit_signal("speak", "There must be a lever to move this log.")
		"push the old log":
			GlobalSignals.emit_signal("speak", "There must be a lever to move this log.")
		look:
...
func set_state(new_state):
	...
	if state == "on":
		tween.tween_property(self, "rect_rotation", 0, log_speed )
		tween.tween_callback(self, "_log_sound_stop")

	else:
		tween.tween_property(self, "rect_rotation", -90, log_speed )
		tween.tween_callback(self, "_log_sound_stop")
