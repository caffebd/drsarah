...
func _sentence_check(sentence):
	...
	match sentence:
		...
		"use the gold cup on the fire":
			GlobalSignals.emit_signal("speak", "I don't want to destroy the cup.")
		"use the gold cup on":
			return
		give:
			...
			GlobalSignals.emit_signal("sentence_clear")
		"look at the gold cup":
			GlobalSignals.emit_signal("speak", "A shiny, golden cup.")
		_:
			if not "shelf" in sentence:
				GlobalSignals.emit_signal("speak", "I can't do that with the cup.")
