...
onready var spawns = [$Demo1MapA/Spawn1]
var spawn_count = 0
...
func check_first_time():
	...
	print ("I am saving the curr level "+GlobalVars.current_level)
...
func setup_room_items():
	...
	print ("game object keys are "+str(game_objects_keys))
...
func _load_game(from_door: bool, from_entry:bool):
	var user_id = FirebaseRest.user_info.id
	print ("so i am loading "+GlobalVars.current_level)
	room_items = yield(FirebaseRest.get_document("EnglishAdventure/"+user_id+"/Game/"+GlobalVars.current_level), "completed")
	print ("room items are "+str(room_items))
	if room_items == null:
